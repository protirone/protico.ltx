...
echo "solution for Uebung $EXC Task 1"

read -r -d '' MV << EOM
Z1F1,Z1F2,Z1F3,Z1F4,active
Z2F1,Z2F2,Z2F3,Z2F4,idle
Z3F1,Z3F2,Z3F3,Z3F4,active
Z4F1,Z4F2,Z4F3,Z4F4,idle

EOM

echo "$MV" | tee my.csv

echo "solution for Uebung $EXC Task 2"
cat my.csv | awk -F, '/active/{print $3, $1}'

echo "solution for Uebung $EXC Task 3"
cat my.csv | awk -F, '/active/{print $3,"-",$1}'

echo "solution for Uebung $EXC Task 4"
cat my.csv | awk -F, '/active/{print $3,",",$1}'

echo "solution for Uebung $EXC Task 5"
cat my.csv | awk -F, '/active/{printf "%s,%s\n",$3,$1}'

echo "solution for Uebung $EXC Task 6"
cat my.csv | grep "active" | sed "s/\(Z[0-9]F[0-9]\),\(Z[0-9]F[0-9]\),\(Z[0-9]F[0-9]\),\(Z[0-9]F[0-9]\)/\3,\1/"
