...
* 4.1 
  ...
  * [X] Alle Rechner eines Subnetzes können die anderen desselben Subnetzes kontaktieren.
  * [ ] Alle Rechner können mit den anderen Rechnern des Firmennetzes kommunizieren.
* 4.2  
...
* 5.1 Beschreiben Sie, was ein LAN ausmacht. (2 Punkte)
...
* 5.1: Ein LAN ist ein mit Kabeln und Switches/Routern generiertes Netzwerk mit der Reichweite eines Camupus. 
* 5.2: 
...
* 6.1 Segmentieren Sie das Netz 172.16.17.0/24 in zwei /25 Netze: Geben Sie für jedes Ihrer neuen Netze Netzadresse, Subnetzmaske und Broadcastdresse an. (4 Punkte)
* 6.2 Segmentieren Sie das Netz 172.16.17.240/29 in zwei /30 Netze: Geben Sie für jedes Ihrer neuen Netze Netzadresse, Subnetzmaske und Broadcastdresse an. (4 Punkte)
...
* 6.1 
  * `172.16.17.0/25`: NA = `172.16.17.0`, NM: `255.255.255.128`, BC = `172.16.17.127`
  * `172.16.17.128/25`: NA = `172.16.17.128`, NM: `255.255.255.128`, BC = `172.16.17.255`
* 6.2 
  * `172.16.17.240/30`: NA = `172.16.17.240`, NM: `255.255.255.252`, BC = `172.16.17.243`
  * `172.16.17.244/30`: NA = `172.16.17.244`, NM: `255.255.255.252`, BC = `172.16.17.247`
* 6.3 
  * `172.16.17.0/25`: GW = `172.16.17.1` oder `172.16.17.126`
  * `172.16.17.128/25`: GW = `172.16.17.129` oder `172.16.17.254`
  * `172.16.17.240/30` GW = `172.16.17.241` oder `172.16.17.243`
  * `172.16.17.244/30` GW = `172.16.17.245` oder `172.16.17.246`
...
Sie finden auf einem Gerät in Ihrem Firmennetz den Hinweis, dass dessen Netzwerkkarte aktuell die Adresse `209e:0:a92c:2323::1/64` nutze. Ihre Chefin - sie ist ein Erich-Kästner-Fan und sagt, ihr Geburtstag sei der 35. Mai - hat Ihnen vorab mitgeteilt, dass Ihr Firmenstandort zwar die Netzadresse `209e:0:a92c:2323::1/48` zugewiesen bekommen habe, dass aber trotzdem alle Geräte in demselben Firmennetz seien. Jetzt möchte sie von Ihnen einen Vorschlag zu Aufteilung in 4 getrennte Netze, von denen jedes einzelne prinzipiell alle Geräte am Standort aufnehmen können sollte. Sie wollen ihr eine persönliche Freude machen. 
...
* 7.1 
  ...
  * Das Präfix `20` kennzeichnet eine Global Unicast Adresse.
  * Global Unicast Adressen sind weltweit gültig = routebar.
  * Ein Präfix `fd` kennzeichnet den nutzbaren Bereich einer Unique Local Adresse:
...
* 7.2 
  * Es gibt im Firmenstandort nur ein Netz mit der Netzadresse `209e:0:a92c:2323::/64`.
...
* 8.2: Neben den /30 Netzen braucht es noch mindestens ein drittes /29 Netz, z.B.
  * `172.16.17.248/29`: NA = `172.16.17.248`, NM: `255.255.255.248`, BC = `172.16.17.255` 
  * mit Hostanzahl 6
