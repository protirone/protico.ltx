---
name: Korrektur
about: Welchen Fehler haben Sie entdeckt?
title: ''
labels: ''
assignees: ''

---

**Was ist Ihnen aufgefallen?**
Ihre klare und prägnante Beschreibung des Fehlers hilft uns sehr:

**In welchem Dokument?**:

* [  ] im Tonspurquellcode `source/lf.??/sbj-??.*/oraltrack.md
* [  ] im Präsentationsquellcode `source/lf.??/sbj-??.*/zp/inc-sbj-??.tex
* [  ] in den allgemeinen drawio-Quellen `source/img.gl`
* [  ] in den lernfeld-spezifischen drawio-Quellen `source/lf.??/img.lf

zu finden mit Suchpattern `xyz`
